...
const sections = [
  ...
  },
  {
    title: "For Employers",
    links: [
      { to: "/employers", label: "Why MedConnect" },
  ...
  return (
    <footer className="relative mt-32 overflow-hidden bg-primary text-primary-foreground">
      ...
      <div className="relative mx-auto max-w-7xl px-4 pt-20 pb-10 lg:px-8">
          ...
          <div>
            ...
            <ul className="mt-7 space-y-3 text-sm text-primary-foreground/80">
              <li className="flex items-start gap-3">
                <span className="mt-0.5 flex h-7 w-7 items-center justify-center rounded-lg bg-primary-foreground/10 text-accent">
                  <MapPin className="h-3.5 w-3.5" />
                </span>
                <span>100 King St W, Suite 4400<br />Toronto, ON M5X 1A4</span>
              </li>
              <li className="flex items-center gap-3">
                <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary-foreground/10 text-accent">
                ...
                <ul className="mt-5 space-y-3 text-sm">
                  {s.links.map((l) => (
                    <li key={l.to}>
                      <Link
                        to={l.to}
                        className="group inline-flex items-center gap-1.5 text-primary-foreground/75 transition hover:text-accent"
                      >
                        {l.label}
        ...
        <div className="mt-16 flex flex-col items-start justify-between gap-4 border-t border-primary-foreground/10 pt-6 text-xs text-primary-foreground/50 md:flex-row md:items-center">
          <p>© {new Date().getFullYear()} MedConnect Canada Inc. All rights reserved.</p>
